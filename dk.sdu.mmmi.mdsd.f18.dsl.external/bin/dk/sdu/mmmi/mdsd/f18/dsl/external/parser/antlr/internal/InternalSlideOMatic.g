...
@after {
	...
	(
		(
			...
			{
				$current = $this_ShowImage_4.current;
				afterParserOrEnumRuleCall();
			}
			    |
			{
			...
			{
				$current = $this_Animation_8.current;
				afterParserOrEnumRuleCall();
			}
		)
		(
...
entryRuleShowImage returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getShowImageRule()); }
	iv_ruleShowImage=ruleShowImage
	{ $current=$iv_ruleShowImage.current; }
	EOF;

// Rule ShowImage
ruleShowImage returns [EObject current=null]
@init {
	enterRule();
}
@after {
